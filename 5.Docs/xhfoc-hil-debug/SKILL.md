...
12. 需要电机转动的硬件在环测试必须先保持电机禁用，并弹出 Windows 准备提示等待用户确认；确认后才发送目标和 `!START`。增加 VOFA 通道时同时核算串口带宽、任务静态栈、运行时高水位和 FreeRTOS 堆余量。
...
- `vofa.csv`：每个有效 JustFloat 帧一行，包含帧序号、接收块时间、原始字节偏移和当前 28 个命名通道。
...
2026-07-26 落地在线多圈梯形轨迹与运动诊断后，当前已烧录并完成无扰动运动 HIL 的候选参数为：

- `positionVelocityLimit = 6 rad/s`，`positionVelocitySafetyLimit = 8 rad/s`。
- `positionAccelerationLimit = 12 rad/s²`，`positionDecelerationLimit = 18 rad/s²`。
- `positionCurrentLimit = 1.2 A`，`positionAccelerationCurrentGain = 0.005 A/(rad/s²)`。
- `positionFrictionCurrent = 0.22 A` 为运动摩擦前馈，`positionStictionCurrent = 0.35 A` 只在低速/起步区平滑介入，`positionDeadband = 0.005 rad`。
- `pidAngle = {P=3.0, I=0, D=0}`，作为轨迹位置误差到速度校正量；`pidVelocity = {P=0.10, I=0, D=0, outputRamp=80}`。
- MT6816 速度改为 `1 ms` 累计位移差分并在窗口间保持结果，避免 10 kHz 单计数差分产生约 `3.8 rad/s` 跳变；使用 `lpfVelocity = 12 ms`。本轮尝试的二阶 PLL 和 `5 ms + 3 ms LPF` 低延迟高带宽组合都出现持续摆动，未通过板测，不得作为当前已验证实现恢复。

位置模式不再把最终目标直接送入角度 P 环。目标改变时从当前轨迹位置/速度在线重规划梯形或三角形轨迹，速度命令由轨迹速度前馈与轨迹位置误差校正组成，再进入速度环；加速度前馈和摩擦补偿与速度反馈输出相加，最后受位置模式总限流约束。轨迹全程使用累计机械角，保持绝对多圈语义。

本轮关键 HIL 证据：

- PLL 与激进 `10 rad/s、30/40 rad/s²、2.5 A` 候选会话 `xhfoc_20260726_162137_739768`：1 rad 阶跃出现约 `3.89–11.13 rad` 持续往返和约 `115.7 rad/s` 峰值，判定失败并 STOP/复位。
- PLL 与保守 `4 rad/s、8/12 rad/s²、1.0 A` 候选会话 `xhfoc_20260726_162813_714477`：仍持续摆动，判定 PLL 动态与当前速度环不匹配并回退。
- 恢复 40 ms 速度低通后的保持会话 `xhfoc_20260726_163402_348261`：6 秒位置跨度约 `0.00038 rad`、峰值速度约 `0.0109 rad/s`、峰值 `|iq|≈0.0448 A`，无自激。
- 保守 1 rad 会话 `xhfoc_20260726_163542_671124`：2% 稳定约 `0.562 s`，无超调，末段 MAE 约 `0.0104 rad`。
- 保守正反两圈会话 `xhfoc_20260726_163831_718754`、`xhfoc_20260726_164110_004333`：两方向均无超调，轨迹约 `3.55 s`，末段 MAE 约 `0.0158–0.0178 rad`。
- 最终 1 rad 会话 `xhfoc_20260726_164431_092113`：2% 稳定约 `0.453 s`，无超调，末段 MAE 约 `0.00651 rad`。
- 最终两圈会话 `xhfoc_20260726_165328_918691`：轨迹约 `2.50 s`，10%–90% 约 `1.719 s`，±0.1 rad 稳定约 `2.375 s`，超调约 `0.0243 rad`，末段 MAE 约 `0.00646 rad`、位置标准差约 `1.83e-5 rad`；峰值 `|iq|≈0.629 A`、峰值电流命令约 `0.731 A`，未触发位置电流、速度命令或电压限幅。
- 顿挫修正最终会话 `xhfoc_20260726_223052_814306`：正反两圈到位约 `3.09/3.23 s`，最终误差约 `+0.0112/-0.0046 rad`；6 rad/s 正反巡航的 `velocity` 标准差约 `1.49/1.47 rad/s`、`iq` 标准差约 `0.114 A`、电流命令标准差约 `0.152 A`，只出现正常的轨迹速度限幅标志。相对旧会话 `xhfoc_20260726_165328_918691`，由位置离线差分得到的 10/20/40 ms 尺度实际速度标准差分别从 `3.448/2.063/0.902` 降为 `1.796/1.156/0.328 rad/s`。当前已验证并烧录的 Release ELF SHA-256 为 `67082BDA6F4F57E89BC1C27538B2831BBD859A19687A5D71DA91543530C8EAD1`。

28 通道 VOFA 在最终会话中约为 `500 Hz`。运行时观测到 `focStackFreeBytes` 最小 `488 B`、`vofaStackFreeBytes` 最小 `588 B`；Release `.su` 中 `ThreadVofa=192 B`、`Motor::CloseLoopControlTick=64 B`、`EncoderBase::GetVelocity=16 B`，当前 FOC/VOFA 任务栈分别为 `1792/1280 B`。最终同步复位会话 `xhfoc_20260726_223828_209606` 显示 `record=valid align=flash`、静止速度/`iq`/电流命令均为零，并进入 `[foc] ready, waiting for !START`。这些证据只证明当前调用路径和测试工况有余量，增加格式化、局部数组或深层调用后必须重新测量。

当前候选尚未执行 `±31.4 rad`、人工扰动、带外部负载以及长时间换向循环，不得把本轮结论扩展为全部工况已验收。若仍能感到与转角同步的细小顿挫，优先检查电机齿槽转矩、MT6816 校验/重复采样和双电阻采样质量，不要继续提高速度 P；若机械速度上限严格，先降低轨迹速度/加速度，不要只提高安全速度限幅。
...
当前 VOFA 周期为 `2 ms（500 Hz）`。每帧包含 28 个小端序 `float32`，随后是帧尾 `00 00 80 7F`，合计 116 字节；理论数据量为 `58 kB/s`，约占 `921600/8N1` 有效载荷带宽的 `62.9%`。通道顺序为：
...
15. `adcRawIb`
16. `adcRawIc`
17. `trajectoryPosition`
18. `trajectoryVelocity`
19. `trajectoryAcceleration`
20. `velocityCommand`
21. `currentCommand`
22. `positionError`
23. `velocityError`
24. `controlLimitFlags`：位 0 轨迹速度限幅、位 1 速度命令安全限幅、位 2 位置模式电流限幅、位 3 电压矢量限幅。
25. `focStackFreeBytes`
26. `vofaStackFreeBytes`
27. `rawVelocity`：编码器窗口差分的未低通速度。
28. `frictionCurrent`：本周期实际加入的位置摩擦前馈电流。

VOFA 任务栈当前为 `1280 B`。28 通道局部数组为 `112 B`，比旧 16 通道版增加 `48 B`；`VofaDebug::SendJustFloat()` 的局部帧缓冲仍按最多 32 通道固定为 `132 B`，不会因实际通道数再次增长。每次 Release 构建后检查 `.su` 静态栈报告，并在板测中确认通道 25、26 的最小剩余字节数；不要仅因能够启动就判断栈安全。FreeRTOS 总堆当前由 `configTOTAL_HEAP_SIZE` 控制，任务创建后还要检查启动日志中的当前/历史最小空闲堆。

围绕帧尾重新同步，并检查候选帧长度和数值合理性。将有效帧之间的 ASCII 字节作为独立日志流处理。在分析报告中标记数据间隙、重连边界、截断帧、NaN 或 Inf，以及元数据错误。

旧的 16 通道 68 字节帧和 26 通道 108 字节帧不能直接使用当前 `decoder_config.json` 解码。分析历史会话时保存并选择与当时固件匹配的通道配置，禁止把旧帧按 28 通道静默重解释。
